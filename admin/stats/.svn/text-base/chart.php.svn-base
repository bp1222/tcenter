<?
	...
	if ($startdate != "" && $enddate != "") {
		$query = "SELECT closeddate, pickupdate FROM tickets WHERE pickupdate >= '$startdate' AND closeddate <= '$enddate';";
	}
	else {
	...
	// Get number of opened and closed tickets in a day
	if ($startdate != "" && $enddate != "") {
	
	    // Get number opened
		$query = "SELECT accepteddate FROM tickets WHERE accepteddate >= '$startdate' AND accepteddate <= '$enddate';";
		$result = pg_query( $query );
		$num_opened = pg_num_rows( $result );
		
		// Get number closed
		$query = "SELECT closeddate FROM tickets WHERE closeddate >= '$startdate' AND closeddate <= '$enddate';";
		$result = pg_query( $query );
		$num_closed = pg_num_rows( $result );
...
<table>
	...
	<?
		if ($startdate != "" && $enddate != "") { ?>
		    <tr><td colspan="2"><b><u>Statistics From <? echo "$startdate To $enddate"; ?></u></b></td></tr>
			...
			<tr>
			    <td><b>Number of Opened Tickets:</b></td>
		        <td><?echo round($num_opened, 2); ?> tickets</td>
			</tr>
			<tr>
			    <td><b>Number of Closed Tickets:</b></td>
		        <td><?echo round($num_closed, 2); ?> tickets</td>
			</tr>
        <?} else {?>
            <tr>
		        <td><b>Average Life Of Ticket:</b></td>
...
<?php
/*
 - Commented out per Dans request.
   jdwrcc - 9-14
<center>
