...
<?
	...
	# set the user to the specified user 
	$queryuser = $passeduname;
...
<?
	...
	if ($name != "" && $username != "" && $address != "" && $email != "" && $owner != "" && $passwd != "" && $inventory != "" && $problemdescription != "") {
		...
		if ($preload != "") {
			...
			verify($name, $username, $phone, $address, $email, $owner, $description, $snumber, $mac, $accountname, $passwd, $inventory, $problemdescription, $preload, $queryuser);

		}
		#if no data was submitted for the machine
		elseif ($preload == "" && $snumber == "" && $description == "" && $mac == "" && $accountname == "" && $passwd == "") {
				echo "Please fill in all required fields when adding a machine.<br>";
				echo "</div></td></tr>";
		}
		...
		echo "<br>Please fill in all of the required information to request assistance.<br>";
		echo "</div></td></tr>";
	}
		...
		<?  
			...
			<? }
				?>
					...
					<tr>
						<td>
							<a href="requesthelp.php#description" target="new"><font color="#ff0000">*</font>Description:</a>
						</td>
						<td style="text-align:right;">
					...
					<tr>
						<td>
							<a href="requesthelp.php#username" target="new">Username to log in with:</a>
						</td>
						<td style="text-align:right;">
					...
					<tr>
						<td valign="top">
							<a href="requesthelp.php#password" target="new"><font color="#ff0000">*</font>Machine account password:</a>
						</td>
						<td style="text-align:right;">
							<input type="text" name="passwd" size="50" maxlength="50" value="<? echo $passwd ?>"><br><b>Note:</b> password is viewable so you can confirm it
						</td>
					</tr>
					<tr>
				...
				<?  ?>
					...
					<tr>
						<td valign="top">
							<a href="requesthelp.php#items" target="new"><font color="#ff0000">*</font>Inventory you are bringing with machine:</a>
						</td>
						<td style="text-align:right;">
					...
					<tr>
						<td>
							<font color="#ff0000">*</font><b><u>Problem Description</u></b>
						</td>
					</tr>
					<tr>
						<td>
				...
				<br><p>Copyright &copy; 2004, 2005 Michael Goffin
...
function verify($name, $username, $phone, $address, $email, $owner, $description, $snumber, $mac, $accountname, $passwd, $inventory, $problemdescription, $preload, $queryuser) {
	...
	if ($do_submit == "yes") {
		submit($name, $username, $phone, $address, $email, $owner, $description, $snumber, $mac, $accountname, $passwd, $inventory, $problemdescription, $preload, $queryuser);
...
function submit($name, $username, $phone, $address, $email, $owner, $description, $snumber, $mac, $accountname, $passwd, $inventory, $problemdescription, $preload, $queryuser) {
	...
	if ($preload != "") {
		...
		$query = "UPDATE machines SET inventory = '$inventory', password = '$passwd', tcnumber = '$ticketid' WHERE description = '$description' AND owner = '$username';";
		$result = pg_query($query);
		if (pg_affected_rows($result) == 1) {
		...
		if ($snumber == "" && $mac == "") {
			$query = "INSERT INTO machines (\"inventory\", \"description\", \"username\", \"password\", \"owner\", \"tcnumber\") values('$inventory', '$description', '$accountname', '$passwd', '$owner', '$ticketid');";
		}
		elseif ($snumber == "" && $mac != "") {
			$query = "INSERT INTO machines (\"inventory\", \"mac\", \"description\", \"username\", \"password\", \"owner\", \"tcnumber\") values('$inventory', '$mac', '$description', '$accountname', '$passwd', '$owner', '$ticketid');";
		}
		elseif ($mac == "" && $snumber != "") {
			$query = "INSERT INTO machines (\"snumber\", \"inventory\", \"description\", \"username\", \"password\", \"owner\", \"tcnumber\") values('$snumber', '$inventory', '$description', '$accountname', '$passwd', '$owner', '$ticketid');";
		}
		else {
			$query = "INSERT INTO machines (\"snumber\", \"inventory\", \"mac\", \"description\", \"username\", \"password\", \"owner\", \"tcnumber\") values('$snumber', '$inventory', '$mac', '$description', '$accountname', '$passwd', '$owner', '$ticketid');";
		}
		$result = pg_query($query);
		if (pg_affected_rows($result) == 1) {
	...
	#we set the date, and the status of the ticket to be new, and the queue to be 	#unassigned
		...
		$query = "INSERT INTO tickets (\"username\", \"tcnumber\", \"date\", \"description\", \"status\", \"progress\", \"queue\") values('$username', '$ticketid', '$thedate', '$problemdescription', '$status', '$progress', '$queue');";
		$result = pg_query($query);
		if (pg_affected_rows($result) == 1) {
			...
			echo "<center><font color=\"#0000ff\">Your request has been logged. Please bring your machine to Resnet along with any items required to fix your issue.</font></center>";
			$success = "success";
